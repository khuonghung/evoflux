...
export function saveWorkflow(workflow: Partial<WorkflowDSL> & { name: string }): WorkflowRecord {
  ...
  const existing = db.prepare('SELECT id FROM workflow WHERE id = ?').get(id) as { id: string } | undefined

  if (existing) {
    db.prepare('UPDATE workflow SET name = ?, description = ?, dsl_json = ?, config_json = ?, updated_at = ? WHERE id = ?')
      .run(workflow.name, workflow.description || '', dslJson, configJson, now, id)
  } else {
    db.prepare('INSERT OR REPLACE INTO workflow (id, name, description, dsl_json, config_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
export function getWorkflow(id: string): WorkflowRecord | null {
  const db = getDatabase()
  const row = db.prepare('SELECT * FROM workflow WHERE id = ?').get(id) as WorkflowRecord | undefined
  return row || null
}

export function listWorkflows(): WorkflowRecord[] {
  const db = getDatabase()
  return (db.prepare('SELECT * FROM workflow ORDER BY updated_at DESC').all() as unknown as WorkflowRecord[])
}
