    ...
    <div style={{ height: 'calc(100vh - 48px)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 40px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 32px 0' }}>Settings</h1>

          <section style={{ marginBottom: 32 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>AI Providers</h2>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <Select value={addType} onChange={setAddType} size="small" style={{ width: 180 }}
                  options={PROVIDER_TYPES.map(t => ({ label: PROVIDER_LABELS[t], value: t }))} />
                <Button type="primary" icon={<PlusOutlined />} size="small" onClick={handleAdd}>Add Provider</Button>
              </div>
            </div>

            {providers.length === 0 && (
              <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-primary)', borderRadius: 12, padding: 40, textAlign: 'center' }}>
                <div style={{ fontSize: 14, color: 'var(--text-tertiary)', marginBottom: 12 }}>No AI providers configured</div>
            ...
            {providers.map((p) => {
              ...
              return (
                <div key={p.id} style={{
                  background: 'var(--bg-elevated)', border: `1px solid ${p.isDefault ? 'var(--accent)' : 'var(--border-primary)'}`,
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{
                        ...
                        background: p.isDefault ? 'var(--accent-muted)' : 'var(--bg-card)',
                      ...
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 6 }}>
                          {p.name}
                          {p.isDefault && <span style={{ fontSize: 9, background: 'var(--accent-muted)', color: 'var(--accent)', padding: '1px 6px', borderRadius: 4 }}>DEFAULT</span>}
                        </div>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      {!p.isDefault && (
                        <button onClick={(e) => { e.stopPropagation(); setDefault(p.id) }}
                          style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-tertiary)', borderRadius: 4 }}
                      ...
                      <Popconfirm title={`Delete ${p.name}?`} onConfirm={() => removeProvider(p.id)} okText="Delete" cancelText="Cancel" okButtonProps={{ danger: true }}>
                        <button onClick={(e) => e.stopPropagation()}
                          style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--error)', borderRadius: 4 }}>
                  ...
                  {isEditing && (
                    <div style={{ padding: '0 16px 16px', borderTop: '1px solid var(--border-primary)' }}>
                      ...
                      {(p.type === 'claude-cli' || p.type === 'copilot-cli') && (
                        <div style={{ marginBottom: 12, padding: 10, background: 'var(--accent-muted)', borderRadius: 8, fontSize: 11, color: 'var(--text-secondary)' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        {testResult[p.id] !== undefined && (
                          <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: testResult[p.id] ? 'var(--success)' : 'var(--error)' }}>
          ...
          <section style={{ marginBottom: 32 }}>
            <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 16px 0' }}>Appearance</h2>
            <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-primary)', borderRadius: 12, padding: 20 }}>

              <div style={{ display: 'flex', gap: 20, marginBottom: 20 }}>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Theme</label>
                  <Select value={appearance.theme} onChange={(v) => updateAppearance({ theme: v })}
                    options={[{ label: 'Dark', value: 'dark' }, { label: 'Light', value: 'light' }]} style={{ width: '100%' }} />
                </div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 32 }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>{appearance.compactMode ? 'On' : 'Off'}</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <Input size="small" value={appearance.accentColor} onChange={(e) => updateAppearance({ accentColor: e.target.value })}
                  placeholder="#0070f3" style={{ width: 140 }} prefix={
                    <div style={{ width: 12, height: 12, borderRadius: 3, background: appearance.accentColor, flexShrink: 0 }} />
                  } />
              </div>
              ...
              <div style={{ display: 'flex', gap: 20, marginBottom: 20 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <Select value={appearance.fontSize} onChange={(v) => updateAppearance({ fontSize: v as FontSize })} style={{ width: '100%' }}
                    options={Object.entries(FONT_SIZE_MAP).map(([k, v]) => ({ label: v.label, value: k }))} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Font Family</label>
                  <Select value={appearance.fontFamily} onChange={(v) => updateAppearance({ fontFamily: v as FontFamily })} style={{ width: '100%' }}
              ...
              <div style={{ display: 'flex', gap: 20, marginBottom: 20 }}>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Node Border Radius: {appearance.nodeBorderRadius}px</label>
                  <Slider min={0} max={20} value={appearance.nodeBorderRadius} onChange={(v) => updateAppearance({ nodeBorderRadius: v })} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Canvas Dot Size: {appearance.canvasDotSize}</label>
              ...
              <div style={{ display: 'flex', gap: 20, marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Canvas Dot Color</label>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    <Input size="small" value={appearance.canvasDotColor} onChange={(e) => updateAppearance({ canvasDotColor: e.target.value })}
                      placeholder="Auto (theme default)" style={{ flex: 1 }} />
                    <button onClick={() => updateAppearance({ canvasDotColor: '' })}
                      style={{ padding: '3px 8px', fontSize: 11, borderRadius: 5, cursor: 'pointer', background: 'var(--bg-input)', border: '1px solid var(--border-primary)', color: 'var(--text-tertiary)' }}>
                      Reset
                    </button>
                  </div>
                </div>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Sidebar Width: {appearance.sidebarWidth}px</label>
          ...
          <section>
            <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 16px 0' }}>About</h2>
            <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-primary)', borderRadius: 12, padding: 20 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-tertiary)' }}>
                Version 1.0.0 &middot; Built with Electron + React + TypeScript
              </div>
            </div>
          </section>
        </div>
